            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
                ...
                <div class="mt-4 sm:mt-0 flex space-x-2">
                    @if(in_array($rental->status, ['pending', 'disetujui']))
                        <form id="cancelRentalForm" action="{{ route('user.rental-history.cancel', $rental->id) }}"
                              method="POST"
                              class="inline">
                            @csrf
                            @method('PATCH')
                            <flux:button type="button" variant="danger" size="sm" icon="x-mark" onclick="confirmCancelRental()">
                                Batalkan Penyewaan
                            </flux:button>
                        </form>
                    @endif
